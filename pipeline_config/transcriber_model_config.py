...
class ModelConfig:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        self.device = config["transcriber_model"].get(
        ...
        self.use_batched_inference = config["transcriber_model"][
            "use_batched_inference"
        ]
        if self.use_batched_inference:
            self.batch_size = config["transcriber_model"]["batched_inference_params"][
                "batch_size"
            ]
            self.num_workers = config["transcriber_model"]["batched_inference_params"][
                "num_workers"
            ]
        else:
            self.batch_size = 4
